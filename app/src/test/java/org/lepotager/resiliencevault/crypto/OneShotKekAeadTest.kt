...
class OneShotKekAeadTest {
    ...
    fun strictEncryptedKeysetParserRejectsUnknownDuplicateAndWrongVersion() = runTest {
        ...
        val wrongVersion = serialized.copyOf()
        wrongVersion[firstEncryptedPayloadOffset(wrongVersion)] = 2
        assertThrows(Exception::class.java) {
            parseStrictEncryptedKeyset(wrongVersion)
        }
    }

    private fun firstEncryptedPayloadOffset(bytes: ByteArray): Int {
        require(bytes.isNotEmpty() && bytes[0] == 0x12.toByte())
        var index = 1
        while (true) {
            require(index < bytes.size)
            val value = bytes[index].toInt() and 0xff
            index += 1
            if ((value and 0x80) == 0) return index
        }
    }
